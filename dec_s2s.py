...
from src.optimizers.optax import get_optimizer, compressed_acc
...
def accumulator_fn(steps, impl="naive", tau=None):
    if impl == "naive":
        return (
            ...
            lambda acc, x, ministep: jax.tree_map(
                lambda ga, g: ga + (g - ga) / (ministep + 1), acc, x
            ),
...
def state_update(grads, state, weight_decay, **kwargs):
    ...
    new_params = jax.tree_map(
        lambda x, y: x - weight_decay * y, new_params, state.params
    )
    return (
        state.replace(
            params=new_params,
...
def main(cfg):
    ...
    def preprocess_function(examples):
        ...
        model_inputs["attention_mask"] = np.zeros_like(
            model_inputs["labels"], dtype=np.int32
        )
        ...
        for i in range(len(contexts)):
            ...
            model_inputs["input_ids"][i][
                1 + len(context_ids[i]) : 1 + len(context_ids[i]) + len(target_ids[i])
            ] = target_ids[i]

            model_inputs["labels"][i][
                len(context_ids[i]) : len(context_ids[i]) + len(target_ids[i])
            ] = target_ids[i]
            model_inputs["labels"][i][
                len(context_ids[i]) + len(target_ids[i])
            ] = tokenizer.eos_token_id

            model_inputs["attention_mask"][i][
                : len(context_ids[i]) + len(target_ids[i]) + 1
            ] = 1
    ...
    def test_preprocess_function(examples):
        ...
        contexts = [
            format_input(cfg.data.dataset_name, inp, prefix) for inp in contexts
        ]
        context_ids = tokenizer(
            contexts,
            max_length=cfg.data.max_source_length - len(sep_ids),
        ...
        model_inputs["input_ids"] = np.asarray(
            [ids + sep_ids for ids in context_ids["input_ids"]]
        )
    ...
    if cfg.training.do_predict:
        ...
        if cfg.data.max_predict_samples is not None:
            max_predict_samples = min(
                len(predict_dataset), cfg.data.max_predict_samples
            )
    ...
    def compute_metrics(preds, decoded_labels):
        ...
        prediction_lens = [
            np.count_nonzero(pred != tokenizer.eos_token_id) for pred in preds
        ]
    ...
    if has_tensorboard and jax.process_index() == 0:
        ...
        for b1_name in b1_candidates:
            if (
                hasattr(cfg.optimizer, b1_name)
                and getattr(cfg.optimizer, b1_name) is not None
            ):
        ...
        for b2_name in b2_candidates:
            if (
                hasattr(cfg.optimizer, b2_name)
                and getattr(cfg.optimizer, b2_name) is not None
            ):
                b2_str = f"-b2({getattr(cfg.optimizer, b2_name)})"
                break
        weight_decay_str = (
            f"-wd({cfg.training.weight_decay})" if cfg.training.weight_decay > 0 else ""
        )
        name = (
            f"{cfg.optimizer.name}{lr_str}{tau_str}{b1_str}{b2_str}{weight_decay_str}"
        )
        ...
        try:
            ...
            summary_writer = tb.SummaryWriter(
                log_dir=Path(cfg.training.output_dir) / name
            )
    ...
    per_device_eval_batch_size = int(
        ...
        or cfg.training.per_device_train_batch_size
    )
    eval_batch_size = (
        per_device_eval_batch_size * jax.device_count() // cfg.training.num_tp_devices
    )
    ...
    optimizer = get_optimizer(
        optimizer_name, learning_rate=linear_decay_lr_schedule_fn, **optimizer_cfg
    )

    devices = np.asarray(jax.devices()).reshape((-1, cfg.training.num_tp_devices))
    mesh = shd.Mesh(devices, ("dp", "tp"))

    params_sharding = get_params_sharding(
        mesh, model.params, model_name=cfg.model.model_name_or_path
    )
    ...
    def train_step(state, batch):
        ...
        if cfg.grad_acc.steps > 1:
            ...
            new_params = optax.apply_updates(state.params, updates)
            new_params = jax.tree_map(
                lambda x, y: x - cfg.training.weight_decay * y, new_params, state.params
            )
    ...
    def eval_step(state, batch, label_smoothing_factor=0.0):
        ...
        logits = model(**batch, params=merge_fn(state.params), train=False)[0]
        loss, num_labels = loss_fn(
            logits,
            labels,
            batch["attention_mask"],
    ...
    num_beams = (
        cfg.data.num_beams if cfg.data.num_beams is not None else model.config.num_beams
    )
    length_penalty = (
        cfg.data.length_penalty
        if cfg.data.length_penalty is not None
    ...
    for epoch in epochs:
        ...
        for step in tqdm(
            ...
            if (
                cur_step % cfg.training.eval_steps == 0
                ...
                for _ in tqdm(
                    ...
                    metrics = eval_step(
                        state, batch, cfg.training.label_smoothing_factor
                    )
                    eval_metrics.append(metrics)

                eval_keys = eval_metrics[0].keys()
                eval_metrics = {
                    k: jnp.stack([metrics[k] for metrics in eval_metrics]).mean()
                    for k in eval_keys
                }
...
def launch(cfg: OmegaConf) -> None:
    ...
    if (
        cfg.data.dataset_name is None
        and cfg.data.train_file is None
        and cfg.data.validation_file is None
    ):
