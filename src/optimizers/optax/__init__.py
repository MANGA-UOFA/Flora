...
import optax
from optax import adamw, lion, sgd
from optax._src import utils as optax_utils
import numpy as np

from src.optimizers.optax.adafactor import adafactor as our_adafactor
from src.optimizers.optax.flora import flora
...
def random_generate(key, shape, dtype=None):
...
def compressed_acc(
    ...
    def accumulate_update(acc_grads, grads, ministep):
        ...
        def _naive_fn(grad, dcomp):
            return NaiveDecomposition(
                data=dcomp.data + (grad - dcomp.data) / (ministep + 1)
            )
        ...
        def _full_layer_fn(grad, dcomp):
            ...
            return TwoSideRandomDecomposition(
                l_data=(l_data + (l_proj_fn(grad) - l_data) / (ministep + 1)).astype(
                    mu_dtype
                )
                if _side != "right"
                else None,
                l_proj=dcomp.l_proj if _side != "right" else None,
                r_data=(r_data + (r_proj_fn(grad) - r_data) / (ministep + 1)).astype(
                    mu_dtype
                )
                if _side != "left"
                else None,
    ...
    def accumulate_build(acc_grads, params):
        ...
        def _layer_fn(dcomp, p):
            ...
            r_proj = (
                random_generate(dcomp.r_proj, (otau, ind), mu_dtype)
                if _side != "left"
                else None
            )
            l_proj = (
                random_generate(dcomp.l_proj, (outd, itau), mu_dtype)
                if _side != "right"
                else None
            )
        ...
        return jax.tree_map(
            ...
            is_leaf=lambda x: isinstance(
                x, (NaiveDecomposition, TwoSideRandomDecomposition)
            ),
        )
